...
class BouncingBallGame:
    def __init__(self, update_time=None,
        ...
        if update_time is None: # so the caller can force
            ...
            update_time = .01
        self.n_collision = 0    # Number of total collisions
    ...
    def run(self, nball=3):
        self.start_ball(nball=nball)    # Setup balls inital properties
        while self.running:
            time.sleep(self.update_time)
            self.screen_update()

    def start_ball(self, nball=1):
        """ Setup balls, size, position, velocity
        ...
        for i in range(nball):
            ...
            ball = Ball(self, radius=100,
                         v_x=v_x, v_y=v_y)
            ball.ball_color_index = randint(
                            0,len(ball.colors)-1)
    ...
    def ball_update(self):
        ...
        if trace_level > 0:
            print("ball_update()")
        #update all balls then display all bals
